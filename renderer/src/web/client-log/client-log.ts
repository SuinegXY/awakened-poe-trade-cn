...
export function handleLine (line: string) {
  ...
  const CN = AppConfig().language === 'cn-Hans' || AppConfig().language === 'cn-Hant'

  if (entry.сhannel === MessageChannel.WHISPER_FROM) {
    if ((match = entry.body.match(TRADE_WHISPER[(AppConfig().realm === 'pc-ggg' && CN) ? 'cn-Hans' : AppConfig().language]))) {
      const [pAmount, pTag] = [
      ...
      // console.log(entry)
    } else if ((match = entry.body.match(TRADE_BULK_WHISPER[(AppConfig().realm === 'pc-ggg' && CN) ? 'cn-Hans' : AppConfig().language]))) {
      const [pAmount, pName] = [
        match.groups!.price.split(' ', 1).toString(),
        match.groups!.price.split(' ').slice(1).join(' ')
      ]
...
const TRADE_WHISPER = {
  ...
  'cn-Hant': /^你好，我想購買 (?<item>.+) 標價 (?<price>.+) 在 (?<league>.+) \(倉庫頁 "(?<tab_name>.*)"; 位置: 左 (?<tab_left>\d+), 上 (?<tab_top>\d+)\)(?<message>.+)?$/,
  'cn-Hans': /^你好，我希望购买你的(?<item>.+)，标价(?<price>.+)，在(?<league>.+) \(市集页 "(?<tab_name>.*)"; 位置: 左起 (?<tab_left>\d+), 上起 (?<tab_top>\d+)\)(?<message>.+)?$/
}
...
const TRADE_BULK_WHISPER = {
  ...
  'cn-Hant': /^你好，我想用 (?<price>.+) 購買 (?<item>.+) in (?<league>.+)\.(?<message>.+)?$/,
  'cn-Hans': /^你好，我希望购买你的(?<item>.+)用于我的(?<price>.+)，在(?<league>.+)(?<message>.+)?$/,
  'ko': /^_FIX_ME_$/
}
